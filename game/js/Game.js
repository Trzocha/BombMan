...
Game = {
  lvlArrayEnemy: [
    //tablica liczebnosci i rodzaju potworow wdg poziomu
    ["balonik", 4], //1
  ...
  lvlArrayBonus: [
    //tablica liczebnosci i rodzaju bonusow wdg poziomu  + prawdopodobienstwo
    ["life", 2, "speed", 1],
  ...
  lvlArrayCrate: [
    //tablica liczebnosci skrzynek wdg poziomu
    30,
    25,
    20,
    15
  ...
  init: function() {
    ...
    document.getElementById("canvasArea").appendChild(Game.canvas);
    ...
    variableDOM.classBtnRight.addEventListener(
      "touchstart",
      function(e) {
        e.stopPropagation();
    ...
    if (VAR.isMobile) {
      //Gdy w menu glownym wybiore sterowanie mobile przy wczytaniu gry laduje przyciski
    ...
    // variableDOM.idUnfold.firstElementChild.addEventListener("click", function(
    //   ev
    // ) {
    //   //wybor sterowania bohaterem
    //   ev.stopPropagation();
    ...
    // variableDOM.idUnfold.lastElementChild.addEventListener("click", function(
    //   ev
    // ) {
    //   ev.stopPropagation();
  ...
  space: function(location, name_enemy) {
    //funkcja sprawdzajaca obszar gdzie mialby pojawic sie enemy, jak obszar jest za blisko enemy wybierany jest nowy
    if (
      (location.x <= 6 && location.y <= 1) ||
      (location.x <= 1 && location.y <= 6) ||
      (location.x <= 3 && location.y <= 3)
    ) {
      //niedopuszczalny obszar pojawienia sie na starcie enemy
      var tmp;
      tmp = Game.board.getEmptySpace();
      Game.space(tmp, name_enemy);
    } else {
      new Enemy(
        location.x * Game.board.fW,
        location.y * Game.board.fH,
        name_enemy
      );
    }
  },
  ...
  lvlUP: function(start) {
    if (!start) {
      //przy pierwszym uruchomienu gry niepotrzebne
      VAR.gameLVL++;
      Game.board = new Board();
...
try {
  setTimeout(function() {
    //zapobiegawczo aby htlm sie wczytał
    Game.init();
  }, 100);
} catch (err) {
  console.log(err);
